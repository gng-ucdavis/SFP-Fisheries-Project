...
##Rough bootstrap for shits and giggles
plot(Fishing.mortality~Mid.length.mm, data=data)
for(i in 1:1000)
{
x=seq(from=min(Mid.length.mm), to=max(Mid.length.mm), by =0.01)
y=rnorm(1,2.96, 0.315)/(1+exp(-abs(rnorm(1, -0.157, .126))*(x-rnorm(1, 108.3, 5.8)))) ###The big issue with this model is that the selectivity coefficient is not strongly signficant. If sel goes positive, the whole s-shape is flipped, whiich is why I 
lines(y~x)
}
